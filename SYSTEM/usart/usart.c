      ...
      if(USART_GetITStatus(USART1, USART_IT_IDLE) != RESET) 
       {
           ...
           recdata.datalen=USART1->DR;//清除中断标志位
           recdata.datalen=USART1->SR;
           recdata.datalen=USART_REC_LEN-DMA_GetCurrDataCounter(DMA2_Stream2);  //收到的数据长度
				 recdata.dataflag=1;
						MYDMA_Enable();
			 }
...
{ 
	...
	if((u32)DMA2_Stream2>(u32)DMA2)//得到当前stream是属于DMA2还是DMA1
	{
  ...
  DMA_InitStructure.DMA_Memory0BaseAddr = (u32)&recdata.databuf;//DMA 存储器0地址
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;//存储器到外设模式
  DMA_InitStructure.DMA_BufferSize = USART_REC_LEN;//数据传输量 
